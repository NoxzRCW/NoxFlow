...
End-to-end tests for NoxFlow using Selenium WebDriver with Python and pytest.
...
- Prompt you to start the NoxFlow app if not running
- Run the tests
...
1. Start Selenium server with Chrome:
   ```bash
   docker run -d --name noxflow-selenium -p 4444:4444 -p 7900:7900 --shm-size="2g" selenium/standalone-chrome:latest
   ```

2. Start the NoxFlow dev server:
   ```bash
...
- Ensure NoxFlow app is running: `curl http://localhost:3000`
...
- Remove existing container: `docker rm -f noxflow-selenium`
